...
exports.createContest = async (req, res, next) => {
  ...
  const user = await User.findOne({ _id: '5f6009435d46b01ceecd35a1' });
...
exports.getContestById = async (req, res, next) => {
  ...
  try {
    if (!contest) {
      return res.status(400).json({ msg: 'This contest ID does not exist' });
    }
    res.status(200).json(contest);
  } catch (error) {
    console.error(error.message);
...
exports.getAllContests = async (req, res, next) => {
  ...
  try {
    if (!contests) {
      return res.status(400).json({ msg: 'There are no contests' });
    }
